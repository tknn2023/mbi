...
impl KNNGraph {
    ...
    // #[inline(always)]
    // fn vec_of(&self, v: usize) -> &[f32] {
    //     // &self.data[v*self.dim..v*self.dim+self.dim]
    //     self.data.get(index)
    // }

    fn tree_search(&mut self, q: &[f32], leaf_size: usize, res_indices: &mut [i32], res_dists: &mut [f32]) -> usize {
        ...
        while self.search_tree.sizes[idx] > leaf_size {
    ...
    pub fn single_query(&mut self, q: &[f32]) -> (Vec<i32>, Vec<f32>, usize) {
        ...
        let mut cnt = self.tree_search(q, self.leaf_size, &mut res_indices, &mut res_dists);
        // cnt = 0;

        
        // let sd = dist(q, &self.data[0..dim]);
        // heap_push(0, sd, &mut res_indices, &mut res_dists);
        // self.cand_indices.push(0);
        // self.cand_dists.push(sd);
        // self.visited[0] = VISITED;
    ...
    // implement for rust
    pub fn new(data: Array2<f32>, n_neighbors: usize, max_candidates: usize,
...
impl KNNGraph {
    ...
    #[staticmethod]
    pub fn dump<'py>(knng: &KNNGraph, path: &str){
    ...
    pub fn load<'py>(path: &str) -> Self{
        ...
        let decoded: KNNGraph = bincode::deserialize(&buffer[..]).unwrap();
        return decoded;
    }

    #[staticmethod]
    pub fn build_graph<'py>(data: PyReadonlyArray2<f32>, n_neighbors: usize, max_candidates: usize,
        max_iters: usize, rpdiv_iter: usize, rpdiv_size_limit: usize, tf_eps: f32, dist: &str)
        -> Self{
        // let data_vec = data.to_vec().unwrap();
        let data_slice = data.as_slice().unwrap();
    ...
    #[pyo3(name="set_query_params")]
    fn py_set_query_params(&mut self, k: usize, eps: f32, leaf_size: usize) {
        self.k = k;
        self.eps = eps;
        self.leaf_size = leaf_size;
    }

    fn set_query_k(&mut self, k: usize) {
        self.k = k;
    }

    #[pyo3(name="single_query")]
    fn py_single_query<'py>(&mut self, py: Python<'py>, q: PyReadonlyArray1<f32>) -> (&'py PyArray1<i32>, &'py PyArray1<f32>, usize){
