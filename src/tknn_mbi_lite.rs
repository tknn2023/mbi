...
use crate::{tknn_sf_mbi::TknnSF, distances::{*}, heap::heap_push};
...
impl TknnMBILite {
    ...
    fn add_node(&mut self, start_idx: usize, end_idx: usize, complete: bool, level: usize) {
        ...
        if complete {
            let mut knng = TknnSF::new(&self.data, start_idx, end_idx+1, self.n_neighbors, self.max_candidates, self.max_iter, self.rpdiv_iter, self.rpdiv_size_limit, self.tf_eps, self.dist.as_str());
    ...
    fn add_data_parallel(&mut self, data: Array2<f32>, label: Array1<i64>) {
        ...
        if forbuild.len() >= 2 { // 조절 가능
            ...
            let threads: Vec<_> = (0..forbuild.len()).map(|i| {
                ...
                thread::spawn(move || {
                    let mut knng = TknnSF::new(&data, start_idx, end_idx+1, n_neighbors, max_candidates, max_iter, rpdiv_iter, rpdiv_size_limit, tf_eps, dist.as_str());
    ...
    pub fn new(tknn_leaf_size: usize, n_neighbors: usize, max_candidates: usize, max_iter: usize, rpdiv_iter: usize, rpdiv_size_limit: usize, tf_eps: f32, dist: &str, k: usize) -> TknnMBILite {
        let blocks = Vec::new();
    ...
    pub fn single_query(&mut self, q: &[f32], start_label: i64, end_label: i64) -> (Vec<i32>, Vec<f32>, usize){
        ...
        for (block_idx, query_type) in queries {
            ...
            if query_type == KNN { //&& level != 0 {
                ...
                let (indices, dists, _cnt) = knng.single_query(&self.data, &self.label, q, start_label, end_label);
                let k = self.k;
                cnt += _cnt;
...
impl TknnMBILite {
    #[staticmethod]
    pub fn dump<'py>(knng: &TknnMBILite, path: &str){
    ...
    pub fn load<'py>(path: &str) -> Self{
        ...
        let decoded: TknnMBILite = bincode::deserialize(&buffer[..]).unwrap();
        return decoded;
    }
    ...
    pub fn build_graph<'py>(data: PyReadonlyArray2<f32>, label: PyReadonlyArray1<i64>, n_neighbors: usize, max_candidates: usize,
        ...
        let mut tknn = TknnMBILite::new(tknn_leaf_size, n_neighbors, max_candidates, max_iters, rpdiv_iter, rpdiv_size_limit, tf_eps, dist, k);
        ...
        println!("TknnMBILite::build_graph: {:?}", s_time.elapsed());
